	...
	<div class="flex justify-center -mx-2">
		...
		<div class="w-1/6 px-2 mt-5">
			<div class="bg-gray-400">
				<a @click="$dispatch('img-modal',{ imgModalSrc:'{{ asset((strpos($image->name, 'https') !== false) ? $image->name : 'storage/images/'.$image->name) }}' })" class="cursor-pointer">
					<img src="{{ asset((strpos($image->name, 'https') !== false) ? $image->name : 'storage/images/'.$image->name) }}"
						alt="{{ (strpos($image->name, 'https') !== false) ? $image->name : 'storage/images/'.$image->name }}"
